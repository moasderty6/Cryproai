...
async def backtest_worker_process(m: types.Message, pool, symbols_to_test):
    ...
    await bot.send_message(
        m.from_user.id, 
        f"⚙️ <b>بدء عملية الباكتيست في الخلفية...</b>\nالعملات المحددة: <b>{symbols_str}</b>\nالهدف: 10% | الوقف: 10%\nيرجى الانتظار، البوت يعمل بشكل طبيعي...", 
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for sym in symbols_to_test:
            ...
            candles = await get_candles_binance(pair, "1h", limit=1000)
            
            if not candles or len(candles) < 800:
                results_log.append(f"⚠️ #{sym}: بيانات تاريخية غير كافية.")
                continue
            ...
            for i in range(750, len(df) - 50):
                ...
                sma20 = df_slice["close"].rolling(20).mean()
                ...
                if score >= 55.0 and confluence >= 2:
                    ...
                    for _, future_candle in future_slice.iterrows():
                        if future_candle["high"] >= tp_target:
                            trade_result = "WIN (10%+)"
                            wins += 1
                            break
                        elif future_candle["low"] <= sl_target:
                            trade_result = "LOSS (10%-)"
                            losses += 1
                            break
    ...
    report = (
        f"📊 <b>نتائج باكتيست رادار NaiF CHarT</b>\n"
        f"━━━━━━━━━━━━━━\n"
        f"🔹 إجمالي الإشارات المستوفية: <b>{total_signals}</b>\n"
        f"✅ الصفقات الناجحة (10% TP): <b>{wins}</b>\n"
        f"❌ الصفقات الخاسرة (10% SL): <b>{losses}</b>\n"
        f"🏆 نسبة النجاح (Win Rate): <b>{win_rate:.1f}%</b>\n"
        f"━━━━━━━━━━━━━━\n"
...
async def run_backtest_cmd(m: types.Message):
    if m.from_user.id != ADMIN_USER_ID:
        return await m.answer("❌ هذا الأمر للأدمن فقط.")
    
    # استخراج العملات من رسالة الأدمن (ما بعد كلمة /backtest)
    args = m.text.split()[1:] 
    
    # إذا لم يكتب الأدمن أي عملة، نطلب منه ذلك
    if not args:
        return await m.answer(
            "⚠️ <b>طريقة الاستخدام الصحيحة:</b>\n"
            "اكتب الأمر متبوعاً برمز العملة أو عدة عملات (مفصولين بمسافة).\n\n"
            "<b>أمثلة:</b>\n"
            "<code>/backtest SOL</code>\n"
            "<code>/backtest PEPE ETH DOGE</code>",
            parse_mode=ParseMode.HTML
        )
    
    # تنظيف الرموز (حذف كلمة USDT لو كتبها بالخطأ، وجعل الحروف كبيرة)
    symbols_to_test = [sym.replace("USDT", "").strip().upper() for sym in args]
    
    # إطلاق الباكتيست كـ Task منفصل وتمرير العملات له
    asyncio.create_task(backtest_worker_process(m, dp['db_pool'], symbols_to_test))
